...
def change_brightness(direction):
    ...
    def _change(qtile):
        try:
            ...
            get = subprocess.run(['brightnessctl', 'get'], capture_output=True, text=True)
            max_b = subprocess.run(['brightnessctl', 'max'], capture_output=True, text=True)
            ...
            # Send notification
            subprocess.Popen([
                'notify-send', '-a', 'Brightness', '-r', '999',
                f'Brightness: {pct}%', '-h', f'int:value:{pct}', '-t', '1000'
            ])
        except:
            pass
    return _change
...
def toggle_redshift():
    ...
    def _toggle(qtile):
        try:
            ...
            # Send notification
            subprocess.Popen([
                'notify-send', '-a', 'Redshift', '-r', '998',
                f'Color Temperature: {state}', '-t', '2000'
            ])
        except:
            pass
    return _toggle
...
keymap = {
    ...
    'A-<Space>': (lazy.spawn("/home/khai/.config/qtile/rofi_app_launcher.sh"), "App launcher"),
    'M-w': (lazy.spawn("thorium-browser"), "Launch thorium-browser"),

    # System controls - Rofi based power menu
    'M-<Escape>': (lazy.spawn("/home/khai/.config/qtile/rofi_power_menu.sh"), "Power menu"),
    ...
    '<F12>': (lazy.spawn("/home/khai/.config/qtile/rofi_keybinds.sh"), "Show keybindings"),
}
...
groups.append(
    ScratchPad("scratchpad", [
        DropDown("term", terminal, opacity=1.0, height=0.6, width=0.6, x=0.2, y=0.2),
        DropDown("music", "spotify", opacity=1.0, height=0.7, width=0.7, x=0.15, y=0.15),
        DropDown("files", "thunar", opacity=1.0, height=0.6, width=0.6, x=0.2, y=0.2),
    ])
)
        ...
        top=bar.Bar(
            [
                ...
                widget.Net(
                    ...
                    mouse_callbacks={'Button1': lambda: qtile.cmd_spawn(f'{terminal} -e btop -p 2')},
                ),
                widget.DF(
                    partition='/',
                    format='󰋊 {r:.0f}% ',
                    ...
                    mouse_callbacks={'Button1': lambda: qtile.cmd_spawn(f'{terminal} -e gdu')}
                ),
                widget.CPU(
                    format="󰻠 {load_percent:.0f}% ",
                    markup=True,
                    foreground=colors[7],
                    padding=5,
                    mouse_callbacks={'Button1': lambda: qtile.cmd_spawn(f'{terminal} -e btop -p 1')}
                ),
                widget.Memory(
                    format="󰍛 {MemPercent:.0f}% ",
                    markup=True,
                    foreground=colors[4],
                    padding=5,
                    mouse_callbacks={'Button1': lambda: qtile.cmd_spawn(f'{terminal} -e btop -p 2')}
                ...
                widget.TextBox(
                    ...
                    foreground=colors[5],
                    padding=5,
                    mouse_callbacks={'Button1': lambda: qtile.spawn("/home/khai/.config/qtile/rofi_power_menu.sh")}
                ),
            ],
            30,
